<?php/**
 * Template Name: Blog
 */
?>
<?php
get_header();
?>
<main class="blog-partage">
    <header>
    <div class="blog-title-container">
        <h2>
            Participer au blog
        </h2>

    </div>
    <div class="blog-subtitle l-col-18 d-col-18 t-col-15 ">
        <h3>
            Lorem ipsum dolor sit amet, consectetuer adipiscing elit, sed diam nonummy nibh euismod tincidunt ut laoreet dolore magna.
        </h3>
    </div>

    </header>

    <div class="header-article l-col-20 d-col-20 t-col-21 m-grid">
        <div class="image-article l-col-9 d-col-9 t-col-21 m-grid">
            <img src="<?php echo get_template_directory_uri(); ?>/images/team.jpg" alt="">
        </div>
        <div class="text-article l-col-10 d-col-10 t-col-21 m-grid">
            <h3>
                IDÉES DE BILLETS
            </h3>
            <p>
                Lorem ipsum dolor sit amet, consectetuer adipiscing elit, sed diam nonummy nibh euismod tincidunt ut laoreet dolore magna aliquam erat volutpat. Ut wisi enim ad minim veniam, quis nostrud exerci tation ullamcorper suscipit lobortis nisl ut aliquip ex ea commodo consequat. Duis autem vel eum iriure dolor in hendrerit in vulputate velit esse molestie consequat.
            </p>
            <a class="btn btn-arrow btn-border">
                <span> <span class="arrow">></span>  Contactez nous<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 36.1 25.8" enable-background="new 0 0 36.1 25.8" xml:space="preserve"><g><line fill="none" stroke="#FFFFFF" stroke-width="3" stroke-miterlimit="10" x1="0" y1="12.9" x2="34" y2="12.9"></line><polyline fill="none" stroke="#FFFFFF" stroke-width="3" stroke-miterlimit="10" points="22.2,1.1 34,12.9 22.2,24.7   "></polyline></g></svg></span>
            </a>
            <div class="call-to-action download">
                <p>

                </p>
            </div>
        </div>


    </div>
    <section class="debat">
        <div class="section-title">
            <h2>
               Participer au debat
            </h2>
        </div>
        <div class="card-container l-col-21 d-col-21 ">
            <div class="card l-col-5 d-col-5 ">
                <img src="<?php echo get_template_directory_uri(); ?>/images/thumb.png" class="icon-card" alt="">
                <div class="card-title">
                    <h3>
                        NOTRE COMMUNAUTÉ
                    </h3>

                </div>
                <div class="card-text">
                    <p>
                        Hey ! <br>
                        vous voulez rejoindre la communauté des amis de la finance ?
                    </p>
                </div>
                <a class="btn btn-arrow btn-border">
                    <span> <span class="arrow">></span>Rejoindre le groupe<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 36.1 25.8" enable-background="new 0 0 36.1 25.8" xml:space="preserve"><g><line fill="none" stroke="#FFFFFF" stroke-width="3" stroke-miterlimit="10" x1="0" y1="12.9" x2="34" y2="12.9"></line><polyline fill="none" stroke="#FFFFFF" stroke-width="3" stroke-miterlimit="10" points="22.2,1.1 34,12.9 22.2,24.7   "></polyline></g></svg></span>
                </a>
            </div>
            <div class="card l-col-5 d-col-5 ">
                <img src="<?php echo get_template_directory_uri(); ?>/images/send.png" class="icon-card" alt="">
                <div class="card-title">
                    <h3>
                        NOTRE COMMUNAUTÉ
                    </h3>

                </div>
                <div class="card-text">
                    <p>
                        Hey ! <br>
                        vous voulez rejoindre la communauté des amis de la finance ?
                    </p>
                </div>
                <a class="btn btn-arrow btn-border">
                    <span> <span class="arrow">></span>  S'inscrire<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 36.1 25.8" enable-background="new 0 0 36.1 25.8" xml:space="preserve"><g><line fill="none" stroke="#FFFFFF" stroke-width="3" stroke-miterlimit="10" x1="0" y1="12.9" x2="34" y2="12.9"></line><polyline fill="none" stroke="#FFFFFF" stroke-width="3" stroke-miterlimit="10" points="22.2,1.1 34,12.9 22.2,24.7   "></polyline></g></svg></span>
                </a>
            </div>
            <div class="card l-col-5 d-col-5 ">
                <img src="<?php echo get_template_directory_uri(); ?>/images/message.png" class="icon-card" alt="">
                <div class="card-title">
                    <h3>
                        NOTRE COMMUNAUTÉ
                    </h3>

                </div>
                <div class="card-text">
                    <p>
                        Hey ! <br>
                        vous voulez rejoindre la communauté des amis de la finance ?
                    </p>
                </div>
                <a class="btn btn-arrow btn-border">
                    <span> <span class="arrow">></span>  Contact<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 36.1 25.8" enable-background="new 0 0 36.1 25.8" xml:space="preserve"><g><line fill="none" stroke="#FFFFFF" stroke-width="3" stroke-miterlimit="10" x1="0" y1="12.9" x2="34" y2="12.9"></line><polyline fill="none" stroke="#FFFFFF" stroke-width="3" stroke-miterlimit="10" points="22.2,1.1 34,12.9 22.2,24.7   "></polyline></g></svg></span>
                </a>
            </div>

        </div>
    </section>
    <section class="action-finance-watch">
        <div class="section-title center t-grid m-col-18 t-col-18 d-col-18">
            <h2>
                PARTICIPER À L'ACTION DE FINANCE WATCH
            </h2>

        </div>
        <div class="card-container-action">
            <div class="member-card l-col-8 d-col-8 t-grid-18  ">
                <h3>
                    DEVENIR MEMBRE DE FINANCE WATCH
                </h3>
                <p>
                    Vous êtes expert en finance ou votre organisation lutte pour une société plus juste, verte et inclusive ? Rejoignez-nous !
                </p>
                <a href="">
                    <img src="" alt="" class="icon">
                    Devenir membre
                </a>
            </div>
            <div class="line border"></div>
            <div class="member-card donation-card l-col-8 d-col-8 t-grid-18  ">
                <h3>
                    NOUS SOUTENIR EN FAISANT UN DON
                </h3>
                <p>
                    Lorem ipsum dolor sit amet, consectetuer adipiscing elit, sed diam nonummy nibh euismod tincidunt
                </p>

                <a class="btn btn-arrow btn-border">
                    <span>Hover me<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 36.1 25.8" enable-background="new 0 0 36.1 25.8" xml:space="preserve"><g><line fill="none" stroke="#FFFFFF" stroke-width="3" stroke-miterlimit="10" x1="0" y1="12.9" x2="34" y2="12.9"></line><polyline fill="none" stroke="#FFFFFF" stroke-width="3" stroke-miterlimit="10" points="22.2,1.1 34,12.9 22.2,24.7   "></polyline></g></svg></span>
                </a>
            </div>

        </div>

        </div>
    </section>

</main>

<?php get_footer(); ?>